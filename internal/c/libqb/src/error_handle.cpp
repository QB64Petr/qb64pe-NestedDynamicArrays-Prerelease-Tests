...
void error(int32_t error_number) {
    ...
    if (error_number == QB_ERROR_DIVISION_BY_ZERO) {
        ...
        if (current_inclercl) {
            snprintf(errmess, sizeof(errmess), "Line: %u (in %s)\nDivision by zero", (unsigned int)current_inclercl,
                     current_includedfilename ? current_includedfilename : "included file");
        } else if (current_ercl) {
            snprintf(errmess, sizeof(errmess), "Line: %u (in main module)\nDivision by zero", (unsigned int)current_ercl);
        } else {
